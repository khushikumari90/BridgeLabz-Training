...
### UC10 — RabbitMQ: A Second Messaging Path
- `RabbitMQConfig` sets up a **Topic Exchange** (`notes-exchange`) with routing key `note.shared`,
  distinct from UC8's JMS direct queue — an Exchange lets more than one consumer react to the same
  event without the Producer knowing they exist.
- `POST /notes/{noteId}/shareNotes` (stub until UC13's real Collaborators exist) — confirms the note
  is owned by the current user, then publishes a `note.shared` event.
- Two independent consumers are bound to the same exchange/routing key:
  `CollaboratorNotifyConsumer` and `ActivityLogConsumer` — the second one was added with **zero
  changes** to the Producer, which is the concrete proof of the routing decoupling benefit.
- Requires a RabbitMQ broker running locally (`spring.rabbitmq.*` in `application.properties`,
  defaults to `localhost:5672`, `guest`/`guest`) — unlike UC8's JMS, there's no embedded option here,
  so RabbitMQ needs to actually be installed/running (e.g. via Docker: `docker run -p 5672:5672 -p 15672:15672 rabbitmq:3-management`).

### UC11 — Spring Batch: Excel Import/Export
- `POST /notes/import` (multipart `file`) — bulk-imports notes from an uploaded `.xlsx` file.
  Expected columns, in order: `title | description | color | typeOfNote`.
- Built as a real Spring Batch chunked Step (`ExcelNoteItemReader` → `NoteImportProcessor` →
  `RepositoryItemWriter`), chunk size 100. `NoteImportService` builds and runs the Job per-request
  since the reader/processor need per-request data (the uploaded file's path, the current user).
- Rows with a blank title are skipped, not written — the response returns accurate
  read/write/skip counts (`ImportResult`) so you can see exactly what happened.
- `GET /notes/export` — downloads the current user's notes as a real, openable `.xlsx` file.
  Uses a **direct Apache POI approach**, no Batch machinery — export is a bounded, single-user
  dataset, so Batch's chunking overhead isn't needed here.
- Spring Batch needs its own tracking tables (`BATCH_JOB_INSTANCE`, etc.) in the same MySQL database —
  `spring.batch.jdbc.initialize-schema=always` auto-creates them; `spring.batch.job.enabled=false`
  stops any job from auto-running on startup, since jobs here are launched explicitly per-request.

### UC12 — Checklist Items on Notes
- `NoteCheckList` entity: itemName, status (`PENDING`/`DONE`), isDeleted (soft-delete flag), note
  (`@ManyToOne` back to the parent `Note`).
- `GET /notes/{noteId}/noteCheckLists` — list a note's (non-deleted) checklist items.
- `POST /notes/{noteId}/noteCheckLists` — add a checklist item (defaults to `PENDING` if no status given).
- `PUT /notes/{noteId}/noteCheckLists/{fk}` — update a checklist item.
- `DELETE /notes/{noteId}/noteCheckLists/{fk}` — soft-deletes a checklist item.
- `POST /notes/{noteId}/noteCheckLists/markAllComplete` — bulk-updates every item on a note to `DONE`
  in one transaction.
- There's no separate owner field on `NoteCheckList` — ownership is enforced entirely at the parent
  `Note` level (reusing `NoteService.getNoteDetail`), since a checklist item's implicit owner is
  always whoever owns the note it belongs to.

## Tech Stack
Spring Boot 3, Spring Security, Spring Data JPA, MySQL, JWT (jjwt), Lombok, Artemis (JMS), Redis,
RabbitMQ, Spring Batch, Apache POI.
...
3. Make sure Redis is running locally (`redis-server`) — needed for UC9's token caching.
   Make sure RabbitMQ is running locally too — needed for UC10 (e.g. `docker run -p 5672:5672 rabbitmq:3-management`).
   JMS (UC8) needs nothing extra, it uses an embedded Artemis broker.
   Spring Batch (UC11) needs nothing extra either — its tracking tables auto-create in the same MySQL DB.
...
- `POST /notes/{id}/shareNotes` with `{ "sharedWithEmail": "someone@test.com" }` → `202`, check app logs for both `[collaborator-notify]` and `[activity-log]` lines from the same event
- `POST /notes/import` with a 50-row `.xlsx` (some rows with blank titles) → check `readCount`/`writeCount`/`skipCount` add up correctly
- `GET /notes/export` → downloads a real, openable `.xlsx` with your notes
- `POST /notes/{noteId}/noteCheckLists` a few items, then `POST /notes/{noteId}/noteCheckLists/markAllComplete` → confirm all items now show `status: "DONE"`
- `DELETE /notes/{noteId}/noteCheckLists/{fk}` then `GET /notes/{noteId}/noteCheckLists` → confirm the deleted item no longer appears
